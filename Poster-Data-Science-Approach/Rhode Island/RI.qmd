---
title: "Data Exploration with 2016 Rhode Island Primary and General Data"
...
# Read CSV file
prim <- read.csv("20160426__ri__primary__president__precinct.csv")
prim <- prim %>% filter(candidate != 'Overvotes' & candidate != 'Uncommitted' & candidate != 'Undervotes') %>% filter(office == 'President')
gen <- read.csv("20161108__ri__general__precinct.csv")
gen <- gen %>% filter(candidate != 'Overvotes' & candidate != 'Uncommitted' & candidate != 'Undervotes' & office == 'President') %>% filter(office == 'President')
```

```{r}
# Count number of distinct precincts of RI
...
ggplot(total_per_precinct_prim, aes(x = total_votes)) +
  ...
  labs(title = "Total Vote Distribution by Precinct RI Primary",
...
ggplot(total_per_precinct_gen, aes(x = total_votes)) +
  geom_histogram(binwidth = 80, fill = "pink", color = "navy") +
  labs(title = "Total Vote Distribution by Precinct RI General",
       x = "Total Votes per Precinct",
       y = "Number of Precincts") +
  theme_minimal()
```
...
```{r}
write.csv(election_data, "election_data_RI.csv", row.names = FALSE)
```
...
alr_drop_na <- election_data %>% 
  filter(party == "Republican") %>% 
  dplyr::select(total_prim, general_share, primary_share) %>%
